...
from ui.text import (
    ...
    ADMIN_ADD_PROMPT_MESSAGE,
    ADMIN_CANCELLED_MESSAGE,
    ADMIN_FINISH_STORE_HINT_MESSAGE,
    ADMIN_INVALID_USER_ID_MESSAGE,
    ADMIN_NO_BROADCAST_USERS_MESSAGE,
    ADMIN_REMOVE_PROMPT_MESSAGE,
    ADMIN_SHARING_EMPTY_MESSAGE,
    ADMIN_STORAGE_NOT_CONFIGURED_MESSAGE,
    ADMIN_STORE_UPLOAD_FAILED_MESSAGE,
    BROADCAST_BUTTON_FORMAT_ERROR_MESSAGE,
    BROADCAST_BUTTON_INVALID_MESSAGE,
    ...
    dashboard_message,
    share_link_generated_message,
    store_item_added_message,
    store_upload_progress_start,
    store_upload_progress_update,
)
...
async def admin_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not is_admin_user(user_id):
        if update.message:
            await update.message.reply_text(ADMIN_ACCESS_DENIED_MESSAGE, parse_mode="HTML")
        return
...
async def handle_admin_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if admin_step == ADMIN_STEP_ADD_ADMIN and is_main_admin_user(user_id):
        try:
            ...
            await update.message.reply_text(
                admin_added_message(admin_id),
                reply_markup=admin_keyboard(True),
                parse_mode="HTML",
            )
        except ValueError:
            await update.message.reply_text(ADMIN_INVALID_USER_ID_MESSAGE, parse_mode="HTML")
        finally:
            reset_user_state(context.user_data)
        return True
    ...
    if admin_step == ADMIN_STEP_REMOVE_ADMIN and is_main_admin_user(user_id):
        try:
            ...
            await update.message.reply_text(
                admin_removed_message(admin_id, removed),
                reply_markup=admin_keyboard(True),
                parse_mode="HTML",
            )
        except ValueError:
...
async def handle_admin_photo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if context.user_data.get(STATE_KEY_ADMIN_STEP) == ADMIN_STEP_STORE_FILES:
        ...
        await update.message.reply_text(
            store_item_added_message("Photo", len(context.user_data.get(STATE_KEY_STORE_FILES, []))),
            parse_mode="HTML",
...
async def handle_admin_video(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if context.user_data.get(STATE_KEY_ADMIN_STEP) == ADMIN_STEP_STORE_FILES:
        ...
        await update.message.reply_text(
            store_item_added_message("Video", len(context.user_data.get(STATE_KEY_STORE_FILES, []))),
            parse_mode="HTML",
            reply_markup=store_creation_keyboard(is_main_admin_user(update.effective_user.id)),
        )
        return True
...
async def handle_admin_document(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if context.user_data.get(STATE_KEY_ADMIN_STEP) == ADMIN_STEP_STORE_FILES:
        ...
        await update.message.reply_text(
            store_item_added_message("File", len(context.user_data.get(STATE_KEY_STORE_FILES, []))),
            parse_mode="HTML",
            reply_markup=store_creation_keyboard(is_main_admin_user(update.effective_user.id)),
        )
        return True
...
def _admins_message() -> str:
    return admins_message(db.list_admins())
...
async def _post_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not user_ids:
        await update.message.reply_text(ADMIN_NO_BROADCAST_USERS_MESSAGE, parse_mode="HTML")
        reset_user_state(context.user_data)
        return
    ...
    await update.message.reply_text(broadcast_started_message(len(user_ids)), parse_mode="HTML")
    ...
    reset_user_state(context.user_data)
    await update.message.reply_text(
        broadcast_complete_message(sent_count, failed_count),
...
async def _finish_store_creation(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    progress_message = await update.message.reply_text(
        store_upload_progress_start(len(files)),
        parse_mode="HTML",
    )
    ...
    for i, item in enumerate(files, 1):
        try:
            if i % 3 == 0 or i == len(files):
                try:
                    await progress_message.edit_text(
                        store_upload_progress_update(i, len(files)),
                        parse_mode="HTML",
                    )
                except Exception:
                    pass
            ...
            logger.error("Error during store creation batch upload: %s", e)
    ...
    with contextlib.suppress(Exception):
        await progress_message.delete()
    await update.message.reply_text(
        share_link_generated_message(total_files, share_link),
        reply_markup=admin_keyboard(is_main_admin_user(user_id)),
        parse_mode="HTML",
    )
